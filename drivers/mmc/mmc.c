...
static int mmc_change_freq(struct mmc *mmc)
{
	...
	if(!hwreset){
		...
		if (err){
      debug("%s: EXT_CSD_RST_N_FUNCTION\n", __func__);
			return err;
		}  
	}else{
      debug("%s: EXT_CSD_RST_N_FUNCTION is set\n", __func__);	
	}  
	if (err)
...
static int mmc_startup(struct mmc *mmc)
{
	...
	if (err)
		printf("SPL-MMC: Fail to switch card speed mode. Use default speed mode!\n");
#else //CONFIG_LS102XA
	if (err)
		return err;
#endif	//CONFIG_LS102XA
