...
static int xhci_set_configuration(struct usb_device *udev)
{
	...
	in_ctx = virt_dev->in_ctx;
	if((udev->descriptor.idVendor == 0x1BC7) && (udev->descriptor.idProduct == 0x1201)){
		//Telit LE910
		num_of_ep = udev->config.if_desc[4].no_of_ep;
		ifdesc = &udev->config.if_desc[4];
	}else if((udev->descriptor.idVendor == 0x1546) && ((udev->descriptor.idProduct == 0x1141) || (udev->descriptor.idProduct == 0x1146))){
		//u-blox /L280/L210
		num_of_ep = udev->config.if_desc[3].no_of_ep;
		ifdesc = &udev->config.if_desc[3];
	}else{
