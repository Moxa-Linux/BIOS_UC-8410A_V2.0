...
/* THE transmit packet */
uchar *NetTxPacket;
uchar  EtherPacket[ETHLOOP_LEN+14];	// buffer for loopback test frame
...
void EthLoopHandler(uchar *pkt, unsigned dest, IPaddr_t sip, unsigned src,
	    unsigned len)
{
  int i,flag=0;
  
  
 //(ETHLOOP_LEN+ETHLOOP_HEAD_SIZE)
...
int EthLoopSend(void)
{
        ...
        for (i=0 ; i<ETHLOOP_LEN ; i++) {
                k = (i%256);
                EtherPacket[ETHLOOP_HEAD_SIZE+i] = k;
        }
        for(i=0;i<6;i++){
                macdec[i]=0xff;
                TestEther[i]=0x11+i;
        ...
        pkt += NetSetEther (pkt, macdec, PROT_TEST); /*set our MAC address as destination address */
        (void) eth_send(EtherPacket, ETHLOOP_LEN+ETHLOOP_HEAD_SIZE);
        printf ("Send OK!\n");
        return 1;       /* waiting */
}

void EthLoopTimeout (void)
{

	eth_halt();
        printf ("time out accur\n");
	net_set_state(NETLOOP_FAIL);	/* we did not get the reply */
}


void EthLoopStart(void)
{
        printf ("Using %s device\n", eth_get_name());
	NetSetTimeout(1000UL, EthLoopTimeout);
	net_set_udp_handler(EthLoopHandler);
        EthLoopSend();
}
