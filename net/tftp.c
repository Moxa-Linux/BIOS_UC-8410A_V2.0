 ...
 */
//R02 11/28/2017   JKW   Added tftp upgrade warning.
//R01 11/28/2017   JKW   Add Firmware Update by Tftp function
...
//R01-start
		if(tftp_upgrade_start){
			fw_tftp_size += len;
			if((NetBootFileXferSize % EMMC_COPY_LIMIT_SIZE) == 0){
				diag_do_copy_download_firmware_to_emmc();
			}
		}
//R01-end
#ifdef CONFIG_MCAST_TFTP
