...
#else	//QSPIboot_Support		
   	for(i=0;i<uboot_size;i+=64){
   		pbl_fget(64, fp_uboot);
   	}
#endif	//QSPIboot_Support
...
#ifdef	QSPIboot_Support
   	add_end_cmd();
   	for (size = pbl_size; size < 0x10000; size++) {
   		*pmem_buf++ = 0xFF;
    		pbl_size++;
    	}
#endif	//QSPIboot_Support
